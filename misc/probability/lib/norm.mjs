...
export const phi = (p) => normsInv(p, 0, 1);
...
function sat() {
  const n = 40,
    ...
    mu = -5 / 6,
    varYi = 25 ** 2 * p * (1 - p);
  (varY = n * 25 ** 2 * p * (1 - p)), (stdY = varY ** 0.5);
  let sn = 400,
    xn = sn / n,
    z = ((xn - mu) * n ** 0.5) / varYi ** 0.5;
  let res = 1 - normalcdf(z);
  pr(`mu: ${mu}`);
  pr(`xn: ${xn}, ${xn - mu}`);
  pr(`stdY: ${stdY}`);
  pr(`z: ${z}  prob of ${sn}: ${res}`);
  pr(`manual z: ${(40 ** 0.5 * (400 / 40 + 5 / 6)) / varYi ** 0.5}`);
